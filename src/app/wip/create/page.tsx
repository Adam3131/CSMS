...
function WipCreateContent() {
  ...
  const [wipNamaPerusahaan, setWipNamaPerusahaan] = useState("");
  ...
  const [wipEvaluator, setWipEvaluator] = useState("");
  const [wipLastEdit, setWipLastEdit] = useState("22 February 2026");

  // Indicator States - WIP Step 2 (Lagging)
  const [wipLagging, setWipLagging] = useState<
    Record<string, { target: string; actual: string; sanksi: string }>
  >({
  ...
  const [wipLeading, setWipLeading] = useState<
    Record<string, { target: string; actual: string; sanksi: string }>
  >({
    row1: { target: "", actual: "", sanksi: "" },
    row2: { target: "", actual: "", sanksi: "" },
  ...
  const [wipPjaIndicators, setWipPjaIndicators] = useState<
    Record<string, { target: string; actual: string; sanksi: string }>
  >({
    row1: { target: "", actual: "", sanksi: "" },
    row2: { target: "", actual: "", sanksi: "" },
  ...
  const [wipLeadingStep3, setWipLeadingStep3] = useState<
    Record<string, { target: string; actual: string; sanksi: string }>
  >({
    row1: { target: "", actual: "", sanksi: "" },
    row2: { target: "", actual: "", sanksi: "" },
  ...
  useEffect(() => {
    ...
    // Auto fill evaluator from currentUser
    const currentUser = getCurrentUser();
    if (currentUser) {
      setWipEvaluator(currentUser.fullName);
    }

    // Auto fill date
    const today = new Date().toISOString().split("T")[0];
    setWipTanggalPenilaian(today);
    ...
    const loadData = async () => {
      ...
      if (companyParam) setWipNamaPerusahaan(companyParam);
      if (locationParam) setWipLokasiPekerjaan(locationParam);

      if (docNoParam && isSupabaseConfigured) {
        const no = parseInt(docNoParam, 10);
        ...
        if (!error && subData) {
          if (subData.nama_perusahaan)
            setWipNamaPerusahaan(subData.nama_perusahaan);
          if (subData.jenis_pekerjaan)
            setWipJenisPekerjaan(subData.jenis_pekerjaan);
          if (subData.lokasi_pekerjaan)
            setWipLokasiPekerjaan(subData.lokasi_pekerjaan);
          if (subData.tanggal_penilaian)
            setWipTanggalPenilaian(subData.tanggal_penilaian);
          if (subData.evaluator) setWipEvaluator(subData.evaluator);
          if (subData.stage) setWipAssessmentStage(subData.stage);

          if (subData.lagging_indicators)
            setWipLagging(subData.lagging_indicators);
          if (subData.leading_indicators)
            setWipLeading(subData.leading_indicators);
          if (subData.pja_lagging) setWipPjaIndicators(subData.pja_lagging);
          if (subData.pja_leading) setWipLeadingStep3(subData.pja_leading);
        }
      }
    };
  ...
  const handleFinishSubmit = async () => {
    ...
    try {
      ...
      if (docNoParam) {
        newDocNo = parseInt(docNoParam, 10);
      ...
      if (!newDocNo) {
        throw new Error(
          "Could not determine document number for WIP submission.",
        );
      }
      ...
      showToast("success", "WIP document saved successfully! Redirecting...");
  ...
  const currentUser = getCurrentUser();
  const userName = currentUser?.fullName || "PUTRI FATIMA SUNNIA";
  const initials = userName
    ...
    .substring(0, 2)
    .toUpperCase();
  const roleDetails = getRoleDetails(currentUser?.role || "User");

  return (
    <>
      <div className="flex min-h-screen bg-slate-50 text-slate-900 font-sans">
        <Sidebar
          currentPath="/wip/create"
          selectedCategory="WIP"
          documents={documents}
        />
          ...
          <main className="flex-1 p-8 space-y-6 max-w-7xl w-full mx-auto animate-fade-in">
              ...
              <div className="flex items-center gap-6 text-xs font-bold">
                {[
                  ...
                  <div key={s.step} className="flex items-center gap-2">
                    ...
                    <span
                      className={
                        wipStep === s.step ? "text-slate-800" : "text-slate-400"
                      }
                    >
                      {s.label}
                    </span>
            ...
            {wipStep === 1 && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                  ...
                  >
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <option value="Stage 1 - Awal Pekerjaan">
                          Stage 1 - Awal Pekerjaan
                        </option>
                        <option value="Stage 2 - Pertengahan Pekerjaan">
                          Stage 2 - Pertengahan Pekerjaan
                        </option>
                        <option value="Stage 3 - Akhir Pekerjaan">
                          Stage 3 - Akhir Pekerjaan
                        </option>
                      </select>
                    </div>
                ...
                {pdfUrl && (
                  <div className="rounded-2xl border border-slate-200/60 bg-white p-1 shadow-sm h-[800px] overflow-hidden flex flex-col">
                  ...
                  <table className="min-w-full border-collapse text-left text-xs font-medium">
                    ...
                    <tbody className="bg-white divide-y divide-slate-300">
                      {[
                        {
                          key: "row1",
                          text: "Fatality atau Oil Spill ≥ 15 Bbls atau Property Damage ≥ USD 1.000.000",
                        },
                        {
                          key: "row2",
                  ...
                  <table className="min-w-full border-collapse text-left text-xs font-medium">
                    ...
                    <tbody className="bg-white divide-y divide-slate-300">
                      {[
                        {
                          key: "row1",
                          text: "Pelaksanaan HSSE Management Walk Through (MWT)/ Manajemen Visit",
                        },
                        {
                          key: "row2",
                          text: "Pemberian reward dan sanksi HSSE",
                        },
                        {
                          key: "row3",
                        ...
                        {
                          key: "row5",
                          text: "Mengikutsertakan pekerja dalam BPJS Ketenagakerjaan",
                        },
                        {
                          key: "row6",
                          text: "Pelaksanaan HSSE Talk/ Tool Box Meeting",
                        },
